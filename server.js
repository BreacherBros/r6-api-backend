...
function getPeak(history, stats, platform) {
  let best = null;
  ...
  /* HISTORY */
  for (const entry of extractHistory(history)) {
...
app.get("/api/stats", async (req, res) => {
  try {
    ...
    const selectedPlatform =
      PLATFORM_MAP[platformType.toLowerCase()];
    ...
    for (const r of results) {
      const p = getPeak(r.history, r.stats, r.platform);
